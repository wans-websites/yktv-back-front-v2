import img1 from "./assets/img/girlcode.png";
import img2 from "./assets/img/bodaboda.png";
import img3 from "./assets/img/lgbtq.png";
import img4 from "./assets/img/sayno.png";
import img5 from "./assets/img/mistif.png";
import img6 from "./assets/img/dearGirl.png";
import img7 from "./assets/img/inside.png";
import img8 from "./assets/img/femaleonly.png";
import img9 from "./assets/img/eastmeet.png";
import img10 from "./assets/img/normalize.png";
import img11 from "./assets/img/whatgirlsprefer.png";
import img12 from "./assets/img/fwb.png";
import img13 from "./assets/img/tillNairobi.png";
import img14 from "./assets/img/caught.png";
import img15 from "./assets/img/iknow.png";
import img16 from "./assets/img/mendont.png";
import img17 from "./assets/img/turnoff.png";
import img18 from "./assets/img/love.png";
import img19 from "./assets/img/adulting.png";
import img20 from "./assets/img/misfits.png";
import img21 from "./assets/img/modern.png";
import img22 from "./assets/img/waiyaki.png";
import img23 from "./assets/img/loresho.png";
import img24 from "./assets/img/cheapest.png";
import img25 from "./assets/img/toyotafj.png";
import img26 from "./assets/img/Mercedes.png";
import img27 from "./assets/img/mazda.png";
import img28 from "./assets/img/porche.png";
import img29 from "./assets/img/ford.png";
import img30 from "./assets/img/brush2.png";
import img31 from "./assets/img/brush3.png";

export const SHOWS = [
  {
    showName: "Streetwise",
    showDescription:
      "Streetwise is a public interview series conducted in Nairobi streets and events with funny and insightful comments about trending and relevant topics in Kenya.",
    showMovies: [
      {
        title: "The Truth Inside Kenya's BodaBoda Business.",
        description:
          "I recently carried out a project as part of a group of creative experts, designers, researchers and urban planners by shooting and producing this documentary for the study on Implementing Creative Methodological Innovations for Inclusive Sustainable Transport Planning, the I-cmiist Prpject.",
        movie:
          "https://drive.google.com/file/d/14OPyNCPZGstTNjZirkLPD2s2v7NH0Gqg/preview",
        trailor: "",
        category: "documentary",
        synopsis: "",
        year: "2023",
        duration: "18:39",
        thumbnail: img2,
        totalViews: 200,
        weeklyViews: [20, 50, 23, 53, 164],
        dailyWatchTime: [60, 340, 345],
        isYKTVshow: false,
      },
      {
        title: "What Kenyans Think About LGBTQ",
        description:
          "Today on Street Wise, Robin asks what Kenyans think about the LGBTQ Community in Kenya and the court ruling.",
        movie:
          "https://drive.google.com/file/d/189bxxJsUFCLmJFt-jB3e5-lwuqdhpuS3/preview",
        trailor: "",
        category: "people & culture",
        synopsis: "",
        year: "2023",
        duration: "12:29",
        thumbnail: img3,
        totalViews: 220,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 34, 234, 24],
        isYKTVshow: false,
      },
      {
        title: "Can Men Say NO To Sex",
        description:
          "In today's Episode, we do a public interview where we ask people whether young men can say no to sex. The answers as expected are crazy and funny but in today's video there is a little more spice to it.",
        movie:
          "https://drive.google.com/file/d/18heU0H6JPq_mWqUuXEi3hqoqQaSjxOVs/preview",
        trailor: "",
        category: "lifestyle",
        synopsis: "",
        year: "2023",
        duration: "12:41",
        thumbnail: img4,
        totalViews: 100,
        weeklyViews: [20, 50, 65, 76],
        dailyWatchTime: [60, 34, 1],
        isYKTVshow: true,
      },

      {
        title: "A MISFITS TALE",
        description: "This is a short film based on a true story .",
        movie:
          "https://drive.google.com/file/d/15LZZi8tgNmmppWvI-HPEb31VHwVfj1ng/preview",
        trailor: "",
        category: "documentary",
        synopsis: "",
        year: "2023",
        duration: "1:30",
        thumbnail: img5,
        totalViews: 10,
        weeklyViews: [20, 50, 43, 43],
        dailyWatchTime: [123, 340, 345],
        isYKTVshow: false,
      },
    ],
  },
  {
    showName: "Lock & Key",
    showDescription:
      "LOCK & KEY is an all fun and laughs podcast hosted by Amy Kinyanjui and Vicky Kuwona about culture, society and some relationship dramas and stories, in Nairobi, Kenya with occasional guest features.",
    showMovies: [
      {
        title: "GIRL CODES 101 FT. JOY OHON",
        description:
          "In today's episode Amy and Vicky feature the lovely Joy Ohon and talk about some of the unspoken girl code rules that females are expected to follow in todays Gen Z society.",
        movie:
          "https://drive.google.com/file/d/1vjsOLgcymu5cQJuifCpeFG7ScUolhvR4/preview",
        trailor: "",
        category: "lifestyle",
        synopsis: "",
        year: "2023",
        duration: "42:43",
        thumbnail: img1,
        totalViews: 230,
        weeklyViews: [20, 50, 45, 24, 23],
        dailyWatchTime: [60, 123, 34],
        isYKTVshow: false,
      },
      {
        title: "GIRLS JUST WANNA HAVE FUN - FT DOMINIC & ELVIS",
        description:
          "In today's episode Amy and Vicky feature the lovely Joy Ohon and talk about some of the unspoken girl code rules that females are expected to follow in todays Gen Z society.",
        movie:
          "https://drive.google.com/file/d/19TUBVKWm3l1PB1w4RwpY1jKA75U2rNMY/preview",
        trailor: "",
        category: "lifestyl",
        synopsis: "",
        year: "2023",
        duration: "1:09:32",
        thumbnail: img6,
        totalViews: 34,
        weeklyViews: [20, 50, 54, 65, 23],
        dailyWatchTime: [60, 340, 55],
        isYKTVshow: true,
      },
      {
        title: "WHAT MEN DON'T KNOW",
        description:
          "Get ready for round two of adulting adventures with Amy, Abigael, and Michelle! In this hilarious and insightful episode, these three ladies spill the beans on even more life lessons they've learned along the way, along with some little secrets women have that men might not know about! From navigating relationships to handling responsibilities, this episode is packed with laughter and wisdom that will leave you nodding in agreement. Don't forget to hit that like, share, and comment button to let us know what videos you want to see next. So sit back, relax, and get ready to enjoy another epic instalment of adulting with a twist!",
        movie:
          "https://drive.google.com/file/d/1fCX71ttND94ev0U-1gaDhds1GzvUG1F4/preview",
        trailor: "",
        category: "lifestyl",
        synopsis: "",
        year: "2023",
        duration: "1:05:09",
        thumbnail: img16,
        totalViews: 34,
        weeklyViews: [20, 50, 54, 65, 23],
        dailyWatchTime: [60, 340, 55],
        isYKTVshow: true,
      },
      {
        title: "Types and Turnoffs in Relationship",
        description:
          "Today on Lock & Key, Amy and Diana host Sam Ndeze as they talk about the different Types and turnoffs in relationships including hygiene, body types and personalities.          .",
        movie:
          "https://drive.google.com/file/d/1qGejuRMsfVlH8YpDmUJSDr1tbeUCZnJu/preview",
        trailor: "",
        category: "lifestyl",
        synopsis: "",
        year: "2023",
        duration: "32:16",
        thumbnail: img17,
        totalViews: 34,
        weeklyViews: [20, 50, 54, 65, 23],
        dailyWatchTime: [60, 340, 55],
        isYKTVshow: false,
      },
      {
        title: "Love At First Sight",
        description:
          "Today on Lock & Key, Amy and Diana host Makori to talk about Deal Breakers in relationships, Appearance, occupation and personality and why they matter when choosing a partner to date.",
        movie:
          "https://drive.google.com/file/d/1T4QOYHl3wssBfBW8D_SiHo_n5HUvHnwU/preview",
        trailor: "",
        category: "lifestyl",
        synopsis: "",
        year: "2023",
        duration: "1:08:20",
        thumbnail: img18,
        totalViews: 34,
        weeklyViews: [20, 50, 54, 65, 23],
        dailyWatchTime: [60, 340, 55],
        isYKTVshow: false,
      },
      {
        title: "ADULTING AS LADIES",
        description:
          "Unlock your mind and unleash your laughter. Today on Lock & Key, Amy is joined by two remarkable guests, Abigael and Michelle, for an exciting discussion on the challenges and triumphs of adulting as women. In this special episode, you'll hear some of the stories of personal growth and resilience that will make you reflect and connect with these incredible women. With thought-provoking insights and humorous anecdotes, this episode promises to be a treat for your mind and soul. And, if you love what you hear, don't forget to Subscribe, share, and comment on your favorite parts.",
        movie:
          "https://drive.google.com/file/d/1dm6EeFSOAjSq73XXooO4L6zfd7l6Ke5p/preview",
        trailor: "",
        category: "lifestyl",
        synopsis: "",
        year: "2023",
        duration: "38:35",
        thumbnail: img19,
        totalViews: 34,
        weeklyViews: [20, 50, 54, 65, 23],
        dailyWatchTime: [60, 340, 55],
        isYKTVshow: false,
      },
    ],
  },
  {
    showName: "Film",
    showDescription: "",
    showMovies: [
      {
        title: "A MISFITS TALE",
        description: "This is a short film based on a true story.",
        movie:
          "https://drive.google.com/file/d/15LZZi8tgNmmppWvI-HPEb31VHwVfj1ng/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "1:31",
        thumbnail: img20,
        totalViews: 143,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 34, 45, 45, 541],
        isYKTVshow: false,
      },
    ],
  },
  {
    showName: "Brush Strokes",
    showDescription: "",
    showMovies: [
      {
        title: "Brush Strokes EP 2 Completed",
        description: "",
        movie:
          "https://drive.google.com/file/d/1GulUzcGtFop2KUk6P7V692t2uqs6T88r/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "18:31",
        thumbnail: img30,
        totalViews: 143,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 34, 45, 45, 541],
        isYKTVshow: false,
      },
      {
        title: "Brush Strokes Ep 3 Completed",
        description: "",
        movie:
          "https://drive.google.com/file/d/1yhInBNwZkL189Vegt0adBpMFFL3W-j0R/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "34:59",
        thumbnail: img31,
        totalViews: 143,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 34, 45, 45, 541],
        isYKTVshow: true,
      },
    ],
  },
  {
    showName: "Cribz 254",
    showDescription:
      "Cribz is a real estate show which features both the glorious and the convenient homes situated in Nairobi, showcasing beauty, design and classes of houses in Nairobi.",
    showMovies: [
      {
        title: "INSIDE A STUDIO APARTMENT IN KILELESHWA",
        description:
          "Today on Cribz 254, Eglah Wandurwa does a house tour of a luxury studio apartment and one bedroom located in Kileleshwa Nairobi.",
        movie:
          "https://drive.google.com/file/d/1HCYF-RI5Zsh_VrHV2RYcUzmTs-Rc4acy/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "8:21",
        thumbnail: img7,
        totalViews: 143,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 34, 45, 45, 541],
        isYKTVshow: false,
      },
      {
        title: "FEMALE ONLY STUDENT HOSTEL IN PARKLANDS",
        description:
          "Today on Cribz 254, Eglah Wandurwa does a House Tour a hostel accommodation for students located in Parklands Nairobi Kenya.",
        movie:
          "https://drive.google.com/file/d/1BqVVDXoSXlqmszt41Wg-2zGlfjsmN4ho/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "6:17",
        thumbnail: img8,
        totalViews: 109,
        weeklyViews: [20, 634, 64, 51],
        dailyWatchTime: [60, 340, 35, 542, 64, 234, 442],
        isYKTVshow: true,
      },
      {
        title: "MODERN FAMILY SIZE APARTMENT IN WESTLANDS",
        description:
          "Today on Cribz 254, Eglah Wandurwa tours a Beautiful three bedroom luxury apartment located in Kileleshwa Nairobi.",
        movie:
          "https://drive.google.com/file/d/1OacHkcuGW6jg9Kg0B6W6weCO9Yalp2G4/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "8:58",
        thumbnail: img21,
        totalViews: 109,
        weeklyViews: [20, 634, 64, 51],
        dailyWatchTime: [60, 340, 35, 542, 64, 234, 442],
        isYKTVshow: false,
      },
      {
        title: "INSIDE WAIYAKI WAY'S NEW APARTMENTS",
        description:
          "Today on Cribz 254,  Eglah Wandurwa takes a look inside the new Waiyaki Way Apartments as she reviews the features, facilities and space.           ",
        movie:
          "https://drive.google.com/file/d/1ItG2bU2BF2OTHVIyrypMl_SQ9x4STSgW/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "6:17",
        thumbnail: img22,
        totalViews: 109,
        weeklyViews: [20, 634, 64, 51],
        dailyWatchTime: [60, 340, 35, 542, 64, 234, 442],
        isYKTVshow: true,
      },
      {
        title: "INSIDE A TOWNHOUSE IN LORESHO",
        description:
          "Today on Cribz 254,  Eglah Wandurwa tours a Four Bedroom Townhouse located in Loresho as she reviews the space and facilities.           ",
        movie:
          "https://drive.google.com/file/d/15S2ShowyjQOgUj2XAku-_hBh66oqL1Kk/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "6:17",
        thumbnail: img23,
        totalViews: 109,
        weeklyViews: [20, 634, 64, 51],
        dailyWatchTime: [60, 340, 35, 542, 64, 234, 442],
        isYKTVshow: true,
      },
      {
        title: "CHEAPEST FAMILY HOME IN KILELESHWA",
        description:
          "Today on Cribz 254, we feature a beautiful three bedroom family home in Kileleshwa. The cheapest in the area.",
        movie:
          "https://drive.google.com/file/d/1aoqkPkehZcsVWOTO9n8Y_dJ3W0O-qeTs/preview",
        trailor: "",
        category: "Real Estate",
        synopsis: "",
        year: "2023",
        duration: "7:33",
        thumbnail: img24,
        totalViews: 109,
        weeklyViews: [20, 634, 64, 51],
        dailyWatchTime: [60, 340, 35, 542, 64, 234, 442],
        isYKTVshow: false,
      },
    ],
  },
  {
    showName: "Auto Flex",
    showDescription: "",
    showMovies: [
      {
        title: "TOYOTA FJ CRUISER UPCLOSE REVIEW",
        description:
          "Attention all adrenaline junkies and off-road enthusiasts! In this thrilling episode of Auto Flex, Albert and Shunet unveil the unrivaled power and rugged charm of the iconic 2011 Toyota FJ Cruiser! Join them as they showcase the FJ Cruiser's muscular design, impressive off-road capabilities, and fearless spirit. With its commanding presence and cutting-edge off-road technologies, this powerhouse is ready to dominate any challenge. Buckle up and join Albert and Shunet as they ignite your wanderlust and unleash the true essence of adventure! ",
        movie:
          "https://drive.google.com/file/d/1aoqkPkehZcsVWOTO9n8Y_dJ3W0O-qeTs/preview",
        trailor: "",
        category: "Comedy, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "7:49",
        thumbnail: img25,
        totalViews: 169,
        weeklyViews: [43, 50, 5, 345, 65, 34],
        dailyWatchTime: [60, 340, 345, 65, 743, 22],
        isYKTVshow: false,
      },
      {
        title: "MERCEDES AMG GLE43 REVIEW",
        description:
          "Satisfy your excitement with this beautiful Mercedes as Albert delves into the breathtaking world of the AMG GLE43. In a comprehensive review and pulse-pounding test drive that captures the essence of power and luxury. Fuel your automotive passion and join us on this epic episode! Like, Share and Subscribe for More.",
        movie:
          "https://drive.google.com/file/d/1dSYAshqR2yZR1lpAz64DwgFHNhUfsjNK/preview",
        trailor: "",
        category: "Comedy, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "7:22",
        thumbnail: img26,
        totalViews: 169,
        weeklyViews: [43, 50, 5, 345, 65, 34],
        dailyWatchTime: [60, 340, 345, 65, 743, 22],
        isYKTVshow: false,
      },
      {
        title: "Mazda CX-5 2018_ Mazda CX-5 review",
        description:
          "Welcome to an exciting journey into the world of automotive excellence with your host, Albert! In this captivating video, we delve deep into the remarkable features and unparalleled performance of the Mazda CX-5 2018 model. If you're a car enthusiast or someone in search of a versatile and stylish SUV, you're in for a treat!",
        movie:
          "https://drive.google.com/file/d/1vsBPYR6tlNbMQoQaQ5sdeoWH0wGL3Vzp/preview",
        trailor: "",
        category: "Comedy, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "8:20",
        thumbnail: img27,
        totalViews: 169,
        weeklyViews: [43, 50, 5, 345, 65, 34],
        dailyWatchTime: [60, 340, 345, 65, 743, 22],
        isYKTVshow: false,
      },
      {
        title: "The Porsche Cayenne 2016",
        description:
          "Are you a Porshe enthusiast? Enjoy this in depth review of the Porsche Cayenne 2016 on this Episode of Auto Flex, as Shunet and Albert compare and view the body and interior of the lovely machine. Subscribe for this and more car reviews weekly.",
        movie:
          "https://drive.google.com/file/d/1f8f7o8pdHF9F5eZ0YGNkqYFq7sWeNEGm/preview",
        trailor: "",
        category: "Comedy, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "8:51",
        thumbnail: img28,
        totalViews: 169,
        weeklyViews: [43, 50, 5, 345, 65, 34],
        dailyWatchTime: [60, 340, 345, 65, 743, 22],
        isYKTVshow: false,
      },
      {
        title: "FORD RANGER WILDTRACK",
        description:
          "Get ready to hit the road with Auto Flex! In our premiere episode, Shunet takes the Ford Ranger Wildtrack for a spin, putting its off-road capabilities and sleek design to the test. Join in the analysis of this pickup truck that's perfect for any adventure.",
        movie:
          "https://drive.google.com/file/d/1nNTzXft8E5Q2JhdPBa7E4FJbUCQRaf8G/preview",
        trailor: "",
        category: "Comedy, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "3:56",
        thumbnail: img29,
        totalViews: 169,
        weeklyViews: [43, 50, 5, 345, 65, 34],
        dailyWatchTime: [60, 340, 345, 65, 743, 22],
        isYKTVshow: true,
      },
    ],
  },
  {
    showName: "Trendy On Topics",
    showDescription:
      "TrendyOnTopics is a teen show around pop culture, trends and funny stories in Nairobi with interviews from personalities and friends, with some controversial topics.",
    showMovies: [
      {
        title: "EASTMEETSWEST ON BEING ATTRACTIVE",
        description:
          "In today's video I host Bobby and Sasha from @eastmeetswest254 as we talk about Dating attractive people, as Bobby and Sasha also share what they like and what's their type as well as what is not. We also talk about the different things that make people attractive and how to attract people.",
        movie:
          "https://drive.google.com/file/d/1DC9d5x5x84fXGV1B9bB7ripcm6NIsLHm/preview",
        trailor: "",
        category: "Comedy, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "27:30",
        thumbnail: img9,
        totalViews: 169,
        weeklyViews: [43, 50, 5, 345, 65, 34],
        dailyWatchTime: [60, 340, 345, 65, 743, 22],
        isYKTVshow: false,
      },
      {
        title: "Cynthia K & Joan Kubai on Normalizing Cheating",
        description:
          "In today’s video I host Cynthia Kariuki and Joan Kubai as we talk about the public display of affection and how to handle cheating in a relationship. In the video Cynthia shares her opinion on cheating and why it affects her how it does as Joan and I grapple with that fact as we try to understand each other’s prescribes on the topic of cheating.",
        movie:
          "https://drive.google.com/file/d/1h4l0isXUkea7ICfVpfpEbq3zwx_ce5AN/preview",
        trailor: "",
        category: "Talk Show, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "26:44",
        totalViews: 196,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 345, 2345, 634, 42],
        isYKTVshow: true,
        thumbnail: img10,
      },
      {
        title: "WHAT GIRLS PREFER_ TrendyOnTopics",
        description:
          "In today’s video I host Stephanie Wanjiku as we talk about What Girls Prefer and how to get the girl you want.",
        movie:
          "https://drive.google.com/file/d/10BZAtDtmFY7HLCqz0jOtEpJoxrrtSflf/preview",
        trailor: "",
        category: "Talk Show, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "21:46",
        totalViews: 196,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 345, 2345, 634, 42],
        isYKTVshow: false,
        thumbnail: img11,
      },
      {
        title:
          "UNDERSTANDING FRIENDS WITH BENEFITS_ SITUATIONSHIPS _ TrendyOnTopics",
        description:
          "In today's video I host Tracy Wathira, Kyle Kiragu and Sarah Seran, as they tell me how they differentiate between their friends and their friends with benefits, as the conversations takes funny unexpected turns when Tracy describes specifically what she looks for and what she likes and as Kyle and I share our experiences with desperate girl in situationships.          ",
        movie:
          "https://drive.google.com/file/d/1jLUvo1_NaXG1nc6_rGTVEwcMlfzzguSk/preview",
        trailor: "",
        category: "Talk Show, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "30:17",
        totalViews: 196,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 345, 2345, 634, 42],
        isYKTVshow: false,
        thumbnail: img12,
      },
      {
        title: "KOT's Favorite Couple - with Jade & Jaduong",
        description:
          "In today's Episode, Raph sits down with Jade and Humprey as they talk about their relationship, handling the haters, relationship struggles and Humprey's twitter wars.",
        movie:
          "https://drive.google.com/file/d/10pomhFP9pU3XwVOhumvTwu3AIQH-QNtL/preview",
        trailor: "",
        category: "Talk Show, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "22:03",
        totalViews: 196,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 345, 2345, 634, 42],
        isYKTVshow: true,
        thumbnail: img13,
      },
      {
        title: "CITY BOY, IDA & IVY ON SEX EDUCATION",
        description: `In today's video I host Ida, Ian and Ivy, as we talk about Sex and Connections, Sex Education and as Ian shares his childhood stories plus how his relationship failed after being "caught in the act". There is more too on the topic of cheating and relationships and how I and my guests hack the tough situations.`,
        movie:
          "https://drive.google.com/file/d/1GWEew92AZpFc9SVLeeWMp18-iB78vM3n/preview",
        trailor: "",
        category: "Talk Show, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "33:01",
        totalViews: 196,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 345, 2345, 634, 42],
        isYKTVshow: false,
        thumbnail: img14,
      },
      {
        title: "CINDY K EXPLAINS HER BIGGEST PRANK",
        description:
          "In today's video I host Cindy Kipsang, the owner of the (I Know Who You Are) Instagram page that got teenagers all over Nairobi excited after she pranked most into thinking she had information about them. An idea that has caused me to call it the biggest prank on Kenyan teenagers because of the huge following she got after the stunt, as well as comments from those who felt scammed and lied to after the prank. She talks about it all, from how the idea came up to why she did it as well as what the future holds for her and her page and her journey as a content creator on youtube using animation as a method of expression.          ",
        movie:
          "https://drive.google.com/file/d/1SQ6p6NK8utLn7G0LqnniHAdRYXNaJreJ/preview",
        trailor: "",
        category: "Talk Show, Lifestyle",
        synopsis: "",
        year: "2023",
        duration: "10:19",
        totalViews: 196,
        weeklyViews: [20, 50],
        dailyWatchTime: [60, 340, 345, 2345, 634, 42],
        isYKTVshow: false,
        thumbnail: img15,
      },
    ],
  },
];
